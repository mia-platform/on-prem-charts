...
REALM_TARGETS := keycloak_realms_master_as_admin_install keycloak_realms_production_install keycloak_realms_install
...
keycloak_realms_production_install:
	@helm dependency build $(WORKING_DIR)
	@cd $(WORKING_DIR) && ./template.sh -r products -e prod -t production
	@docker run \
		--rm \
		--network host \
		--add-host auth.mia-platform.test:127.0.0.1 \
		-v $(WORKING_DIR)/rendered/prod/production/products:/configs \
		-e KEYCLOAK_URL="$(KEYCLOAK_URL)" \
		...
		-e IMPORT_FILES_LOCATIONS="/configs/*.yaml" \
		docker.io/adorsys/keycloak-config-cli:latest-26.5.4
	@cd $(WORKING_DIR) && ./template.sh -r extensibility -e prod -t production
	@docker run \
		--rm \
		--network host \
		--add-host auth.mia-platform.test:127.0.0.1 \
		-v $(WORKING_DIR)/rendered/prod/production/extensibility:/configs \
		-e KEYCLOAK_URL="https://auth.mia-platform.test" \
		-e KEYCLOAK_CLIENT_ID=keycloak-config-cli \
		-e KEYCLOAK_CLIENT_SECRET=change_me \
		-e KEYCLOAK_GRANT_TYPE=client_credentials \
		-e KEYCLOAK_SSLVERIFY="false" \
